...
export default function AccountUpdate() {
    ...
    useEffect(() => {
        //Cuando el producto seleccionado cambia se cargan sus datos
        if (selectedProduct) {
    ...
    useEffect(() => {
        // Se comprueba si el formulario tiene cambios en comparación con el estado anterior
        if (originalData) {
            const isChanged =
    ...
    const handleReset = () => {
        if (originalData) {
            ...
            //Si hay imagen en los datos originales se resetea su previsualización
            const product = products.find(p => p.id === parseInt(selectedProduct.id));
            if (product) {
                setImagePreview(product.image);
            }
