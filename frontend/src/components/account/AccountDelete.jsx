// Importa hooks de React y algunos íconos de lucide-react
import {useEffect, useState} from 'react';
import {Trash2, Search, X, ChevronDown} from 'lucide-react';
import {
    API_URL, useFetchProducts, useFetchIsMobile, supabase, useProductState, handleProductSelect
} from "../../hooks/Hooks.jsx";
import {categories} from "../../categoriesList.js"


// Componente principal para eliminar productos desde la cuenta
export default function AccountDelete() {

    // Estados locales para controlar el filtro, el modal y la categoría
    ...
    // Obtiene y gestiona el estado global de productos mediante un custom hook
    const {
        products,
        setProducts,
        searchTerm,
    ...
    useEffect(() => {
        setFilteredProducts(// Filtra productos según búsqueda y categoría seleccionada
...
// Confirma y ejecuta la eliminación del producto seleccionado
    ...
    const handleConfirmDelete = async () => {
        ...
        setDeleteStatus({
            show: true, message: `"${selectedProduct.name}" has been deleted successfully`, isError: false
        });
...
// Limpia la selección del producto y cierra el modal

    const handleClearSelection = () => {
        setSelectedProduct(null);
        setShowModal(false)
    };
// Vuelve a cargar productos y detecta si es móvil
...
// Renderizado principal del componente
    ...
    return (<>
        ...
        {<div
            ...
            {filteredProducts.length > 0 && !showModal ? (<div className="mb-6 ">
                <div className="block sm:hidden ">
                    <div className="grid gap-4 " style={{maxHeight: '60vh', overflowY: 'auto'}}>
                        {filteredProducts.map((product) => (<div key={product.id}
                            ...
                            <div
                                className="text-sm text-gray-500 mb-1 dark:text-zinc-50">Category: <span
                                className="text-gray-900 dark:text-zinc-200">{product.category}</span>
                            </div>
                            <div className="text-sm text-gray-500 mb-1 dark:text-zinc-50">Price: <span
                                className="text-gray-900 dark:text-zinc-200">${product.price.toFixed(2)}</span>
                            </div>
                            <div className="text-sm text-gray-500 mb-2 dark:text-zinc-50">Stock: <span
                                className="text-gray-900 dark:text-zinc-200">{product.stock}</span>
                            </div>
                            <button
                    ...
                    <div
                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-zinc-500">
                            ...
                            {filteredProducts.map((product) => (<tr key={product.id}
                                                                    className="hover:bg-gray-50 dark:bg-zinc-800  dark:hover-bg-zinc-700">
            ...
            </div>) : (!showModal && <div className="py-6 text-center bg-gray-50 rounded-lg border border-gray-200">
                ...
                <div className="fixed inset-0 flex items-center justify-center z-40 bg-black bg-opacity-50 ">
                    <div
                        ...
                        <div
                            ...
                            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-200">Selected
                                Product</h3>
                            <button
