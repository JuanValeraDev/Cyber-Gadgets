...
export default function Chatbot() {
    ...
    // If closed, just show the chat button
    if (!isOpen) {
        return (
    ...
    // Chat is open - either in compact or full screen mode based on screen size
    return (
        <div
            className={`fixed z-50 ${
                isMobile
                    ? "inset-0 m-0 rounded-none"
                    : "bottom-6 right-6 w-80 h-96 rounded-lg"
            } flex flex-col bg-white dark:bg-zinc-800 shadow-xl border dark:border-zinc-700`}
        >
            {/* Chat header */}
            <div className="px-4 py-3 bg-primary text-white dark:bg-primary-dark flex justify-between items-center">
            ...
            {/* Chat input */}
            <form onSubmit={handleSubmit} className="p-3 border-t dark:border-zinc-700 flex">
                <input
                    type="text"
                    value={inputValue}
                    onChange={(e) => setInputValue(e.target.value)}
                    placeholder="Type your message..."
                    className="flex-1 px-3 py-2 border rounded-l-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-zinc-700 dark:border-zinc-600 dark:text-white"
                />
                <button
                    type="submit"
